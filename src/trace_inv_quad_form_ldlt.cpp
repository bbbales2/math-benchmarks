...
static void trace_inv_quad_form_ldlt(benchmark::State& state) {
  ...
  auto init = [](benchmark::State& state) {
    ...
    x_ldlt(CAST_VAR(x_val));
    
    return std::make_tuple(x_ldlt, CAST_VAR(y_val));
  };
...
static void trace_inv_quad_form_ldlt_vector(benchmark::State& state) {
  using stan::math::var;
  using stan::math::promote_scalar;
...
BENCHMARK(trace_inv_quad_form_ldlt_vector)->RangeMultiplier(2)->Range(start_val, end_val)->UseManualTime();
BENCHMARK_MAIN();
